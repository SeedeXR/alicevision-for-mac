...
SimStatOps::SimStatOps(av::gpu::Device& dev)
    : impl_(std::make_unique<Impl>(dev,
                                   dev.make_pipeline("av_simstat_validate")))
{}

SimStatOps::SimStatOps(SimStatOps&&) noexcept            = default;
SimStatOps& SimStatOps::operator=(SimStatOps&&) noexcept = default;
SimStatOps::~SimStatOps()                                = default;

void SimStatOps::validate(std::span<const float> inputs,
                          std::span<float>        outputs)
{
    if (inputs.size() % kInPerCase != 0)
        throw std::invalid_argument("SimStatOps: inputs size mismatch");
    const std::size_t count = inputs.size() / kInPerCase;
    if (outputs.size() != count * kOutPerCase)
        throw std::invalid_argument("SimStatOps: outputs size mismatch");
    ...
    in_buf .set_label("simstat.in");
    out_buf.set_label("simstat.out");
